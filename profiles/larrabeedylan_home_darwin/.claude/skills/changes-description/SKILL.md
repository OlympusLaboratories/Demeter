# PR Description Generator — Adversarial Draft Swarm

Produce a pull request description by making four reviewers with incompatible goals fight over it until they all sign off. A senior engineer argues for accuracy and completeness, an impatient reviewer argues for brevity, a novice argues for plain language, and a hawk-eyed manager hunts sentences that were generated rather than thought about. An editor referees and settles the through-line — the one sentence naming what this PR is about; a composing editor then builds the prose on that spine against a mechanically measured word budget; and a fact-checker confirms the result is true about the diff and lost nothing on the way.
...
Write for a real human reviewer who **did not write this code, does not have full context on the initiative it's part of, and gets overwhelmed by a jargon-y wall of text.** They may not fully understand the architecture being modified. They should be able to read the description top to bottom, once, and come away able to state what the PR does and why — without opening a design doc, a ticket, or a sibling PR.
...
- **Ticket / initiative context** if any (identifier, **its URL**, title, one line of what the initiative is for). Pass the URL even when it looks obvious — without it the description falls back to a bare `PLAT-1234`, and two PRs from the same week end up formatting the same ticket two different ways.
- **Anything a sibling PR established that this one leans on** — a limit, a cap, a failure mode, a number. The reader of *this* description has not read that one, so the packet must carry the fact itself, not a pointer to it.
...
- **Anything already known to be out of scope, deferred, or following in a later PR.**
...
export const meta = {
  ...
  description: 'Four adversarial personas fight over a PR description; an editor referees',
  phases: [
    { title: 'Draft' },
    { title: 'Cross-examine' },
    { title: 'Synthesize' },
...
Hard rules:
1. Summary and Changes are the whole description in a normal PR. Testing and Rollout are
...
9. THE SUBSTITUTION TEST: every sentence must be FALSE of some other pull request. If a
...
11. THE THROUGH-LINE. Every description has a spine: one sentence naming what this PR is
   about. The Summary expresses it, and every bullet in Changes must visibly serve it —
   a reader should be able to say why each bullet is in the same PR as the others. The
   ...
   PR has. Order the bullets by something real — the order the reviewer will meet them
   ...
   A Summary sentence shaped like "this PR does A, B and C" is a MANIFEST, not a spine —
   that list is what Changes is for. Three things in one PR are usually three places one
...
13. OPERATIONAL FACTS GO IN THEIR OWN SECTION. A fact about how this must be deployed —
   an ordering, a flag, a required config value, a manual step, a separate PR that must
...
14. NOT-DONE FACTS. A claim about code that did NOT change, or work deliberately left out
   — a gap this PR does not close, a follow-up, something not wired into the build — earns
...
context from a ticket, design doc, or sibling PR that is not restated here — including a
...
TOGETHER — you cannot say why these changes belong in one PR, a bullet's relation to the
...
CUTTING, not explaining — say which. A gloss added to a list makes a longer list.`,
  },
  {
    ...
    charter: `You have read four hundred pull requests this year and you can tell inside two
...
confident sentences that would be equally true of a hundred other PRs; benefits asserted
...
DIFFERENT pull request — apply that substitution test literally, it is your sharpest tool;
...
hundred other PRs ("improves the reliability of the grant system"). A generic spine is the
...
const DESC_SCHEMA = {
  ...
  properties: {
    ...
    // The spine: one sentence naming what this PR is about, chosen BEFORE the
...
const drafts = (await parallel(PERSONAS.map((p) => () =>
  agent(
    `You are ${p.title}. You are one of ${PERSONAS.length} drafters in an adversarial contest to write the pull request description for the change below.
...
Below are ${PERSONAS.length} candidate PR descriptions — one from each persona, including yours. Attack the ones you did not write, and name what your own gets wrong if you can see it. You are deciding what must survive into the final text and what must die.
...
while (round < ROUNDS) {
  ...
  const verdicts = (await parallel(PERSONAS.map((p) => () =>
    agent(
      `You are ${p.title}, judging round ${round} of an adversarial contest over this PR description.
...
- Build the narrative on the spine: what was broken, what this PR does about it, what a reviewer needs to know to read the diff. Each bullet must earn its place against the spine, and its relation to the spine should be evident from its first few words.
...
- Keep the tense honest about what is already true. Today's behavior written in the plain present ("every grant becomes a conditional binding") reads for a beat as something this PR introduces — "today, every grant is a conditional binding" costs one word and removes the double-take.
...
- Operational facts (ordering, flags, required config, a manual step, a separate PR that must land first) move OUT of Changes bullets and into Rollout. That is a move you are allowed and expected to make: it is shape, not content, the fact itself is unchanged, and Rollout is paid for from its own allowance.
...
const fact = await agent(
  `You are the fact-checker on a PR description. You are not a stylist and you have no opinion about length or tone — do not raise any.
...
**Output `description` inside a single markdown code block** (triple backticks, no language tag) so the user copies raw markdown straight into GitHub's description field. Nothing else goes inside that block — no preamble, no notes, no debate.
...
6. **The description ships alone.** Everything outside the code block is for the user's eyes, never for pasting into the PR.
