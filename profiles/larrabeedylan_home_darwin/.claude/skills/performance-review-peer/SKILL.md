...
The peer name is provided as an argument (e.g., `performance-review-peer mark`). Read all weekly profiles in `context/{peer_name}/{START_DATE}_to_{END_DATE}.md`. These are written by the `eng-snippet` skill and contain Slack interactions, MR reviews, shared Linear tickets, and collaboration summaries specific to that peer.
