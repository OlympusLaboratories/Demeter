...
Read a GitHub pull request and help someone who **did not write these changes** genuinely understand them — starting from the ground up, told as a story, easing in gently, going deeper only where they want to.
...
- **Tell a story, don't dump a spec.** There's a reason this PR exists, a problem in the world, a before and an after. Walk them through it like a narrative, not a categorized inventory of files.
...
1. **Read the PR description carefully.** It's the author's own explanation and the single best source of *why*. Note linked tickets, motivation, context.
...
5. **Find the one hard idea.** Identify the single concept that, once it clicks, makes the rest of the PR obvious. That concept is where your explanation should spend its patience. Also note any reviewer debates — unresolved `inline` threads and `CHANGES_REQUESTED` reviews mark the genuinely tricky parts.
