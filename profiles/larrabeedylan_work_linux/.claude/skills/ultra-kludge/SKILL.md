...
if (!areas.length) {
  log('ultra-kludge: no feature areas provided — nothing to review')
...
if (!rawProposals.length) {
  log('ultra-kludge: no kludge surfaced across the reviewed areas')
...
log(`ultra-kludge: ${areas.length} areas, ${rawProposals.length} raw -> ${consolidated.length} consolidated -> ${survivors.length} survived ${VOTES}-vote critique`)
...
```
🧹 ultra-kludge swarm launched — run ID: wf_ab12cd34
...
If any issue was encountered, **edit this skill file** (`~/.claude/skills/ultra-kludge/SKILL.md`) with a surgical fix so the next run is better. Briefly tell the user what you changed and why.
