...
Read the file `~/.claude/skills/eng-snippet/slack-context.md` using the Read tool. This file contains Slack threads, discussions, and other context that Dylan has pasted before running this command.
...
Before summarizing into the concise snippet, write a detailed weekly report to `~/.claude/skills/performance-review-self/context/`. This report is **not** for Slack — it's a rich record for later performance review analysis.

### File naming

Write the report to `~/.claude/skills/performance-review-self/context/{START_DATE}_to_{END_DATE}.md` (e.g., `2026-02-13_to_2026-02-20.md`).
...
After writing Dylan's self-report, generate a collaboration profile for each peer Dylan interacted with this week. These profiles feed the `performance-review-peer` skill at review time.
...
For each peer, write a profile to `~/.claude/skills/performance-review-peer/context/{peer_name}/{START_DATE}_to_{END_DATE}.md`. Create the `{peer_name}/` subdirectory if it doesn't exist.
...
- **Keep it factual.** These profiles are raw evidence — save editorializing for the `performance-review-peer` skill at review time.
...
If any issues were encountered, **edit this skill file** (`~/.claude/skills/eng-snippet/SKILL.md`) to add instructions, warnings, or tips that would prevent the same issue next time. Keep edits surgical — add a note near the relevant step rather than rewriting sections. Briefly tell the user what was updated and why.
