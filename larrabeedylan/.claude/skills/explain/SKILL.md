...
8. **Don't editorialize.** This skill explains, it doesn't review. Don't critique the code or suggest improvements — that's what the address-comments skill is for.
...
If any issues were encountered, **edit this skill file** (`~/.claude/skills/explain/SKILL.md`) to add instructions, warnings, or tips that would prevent the same issue next time. Keep edits surgical. Briefly tell the user what was updated and why.
