...
## Step 3e: Detect "address PR comments" tickets

After displaying the ticket, check if the ticket is about **addressing comments or feedback left on a pull request**. Look for signals like:
- Title or description mentions "address comments", "address feedback", "follow up" on a pull request/PR
- Description contains a GitHub PR URL (e.g., `https://github.com/.../pull/NNN`)

If the ticket matches this pattern:
1. Extract the PR URL from the description.
2. **Invoke the `pr-description` skill** using the `Skill` tool, passing the PR URL as the argument.
3. After the skill completes, skip Steps 4 and 5 (plan and implement) — the address-comments skill handles everything.
4. Proceed directly to Step 6 (commit message and PR description).

If the ticket does NOT match this pattern, continue to Step 4 as normal.

## Step 4: Plan the Implementation

Now enter plan mode using `EnterPlanMode`. Use the ticket title, description, comments, and any labels/context to:
...
1. Invoke the `commit-message` skill using the `Skill` tool to generate a commit message.
2. **Immediately** invoke the `pr-description` skill using the `Skill` tool to generate a PR description. Do NOT wait for the user to respond to the commit message first — both outputs must be presented in the same turn, back-to-back. The `commit-message` skill ends with a suggestion to run `/commit`, but you must continue and invoke `mr-description` before yielding to the user.

## Important Rules

1. **NEVER modify external state beyond git branch operations.** Do not update Linear tickets, push branches, or create PRs unless explicitly asked.
...
- Did the commit-message and pr-description skills produce accurate output on the first try, or did the user need to correct them?

If any issues were encountered, **edit this skill file** (`~/.claude/skills/fix/SKILL.md`) to add instructions, warnings, or tips that would prevent the same issue next time. Keep edits surgical — add a note near the relevant step rather than rewriting sections. Briefly tell the user what was updated and why.
